  ...
  async (req, res) => {
    ...
    try {
              ...
              if (!parsed) {
                ...
                if (hasCsrf || hasSession) {
                  ...
                  if (currentFlow && flowMeta?.state === state) {
                    ...
                    logger.warn(
                      '[MCP OAuth] Skipping failFlow for superseded OAuth error callback',
                      {
                        flowId,
                      },
                    );
      ...
      const isStalePendingFlow =
        ...
        (!currentFlowState.createdAt ||
          Date.now() - currentFlowState.createdAt >= PENDING_STALE_MS);
        ...
        const tokens = await MCPOAuthHandler.completeOAuthFlow(
          ...
          async (exchangedTokens, completePersistedFlow) => {
            ...
            try {
              ...
              logger.error(
                '[MCP OAuth] Failed to store OAuth tokens before flow completion',
                error,
              );
              throw error;
            }
        ...
        try {
          if (flowState.userId !== 'system') {
            ...
            if (requestScoped) {
              ...
              logger.debug(
                `[MCP OAuth] Attempting to reconnect ${serverName} with new OAuth tokens`,
              );
