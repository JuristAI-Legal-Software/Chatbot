...
// SeriesAI tools (triggerSafeDraft, sendRoundUpdate, queryCapTableOwnership,
// scheduleComplianceDeadline, requestSignaturesViaEmail,
// inviteExternalParticipant) are exposed via the juristai-django MCP server
// (mcp_server/server.py) like every other JuristAI tool call -- discovered
// dynamically through the same MCP mechanism, not pre-registered here. Only
// the system-prompt context injection stays in this file.
...
export const SERIESAI_DISCLAIMER =
  "SeriesAI is Venture Intelligent AI. It is not a law firm and does not provide legal advice. It is not a broker-dealer, investment adviser, or financial-services firm and does not offer or recommend securities. All legal, tax, and investment decisions remain the user's responsibility.";
export function hasSeriesAIContext(
  context: SeriesAIContext | undefined,
): context is SeriesAIContext & Required<Pick<SeriesAIContext, 'appId' | 'organizationId'>> {
  return (
    ['3', '4'].includes(String(context?.appId ?? '').trim()) &&
    Boolean(context?.organizationId?.trim())
  );
}
